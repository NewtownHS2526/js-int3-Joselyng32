...
let Jordangames = Math.floor(eachSiblingGets / game);
...
let saleShoes = shoes * 0.5;
console.log(newGameprice);
console.log(saleShoes);
// 4. Quinn and Ricardo are twins. If they put their money together, how many on sale shoes can they buy in a month (4 weeks)?
let Twinsmoney = eachSiblingGets * 2 * 4;
let shoesInMonth = Math.floor(Twinsmoney / saleShoes);
console.log(shoesInMonth);
// 5. How many more games can they buy if they put their money together compared to if they had bought games on their own?
let QuinnGames = Math.floor(eachSiblingGets / newGameprice);
let RicardoGames = Math.floor(eachSiblingGets / newGameprice);
let combinedGames = Math.floor((eachSiblingGets * 2) / newGameprice);
let moreGames = combinedGames - (QuinnGames + RicardoGames);
console.log(moreGames);
// 6. Print out the three siblings' names. "The three siblings are __, __, and ___."
let siblingNames = `The three siblings are ${sibling1}, ${sibling2}, and ${sibling3}.`;
...
if( budget >= mealCost) {
    ...
    let moneyLeft = (budget - (numMeals * mealCost)).toFixed(2);
    return `You can buy ${numMeals} meals and have $${moneyLeft} left over.`;}
else {
...
function missingLeg(side1, side2) {
    ...
    let missingLegLength = Math.sqrt((hypotenuse * hypotenuse) - (leg * leg));
    return missingLegLength;
}
